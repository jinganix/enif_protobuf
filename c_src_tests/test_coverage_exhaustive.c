...
test_nested_oneof_deep(void)
{
    ...
    if (setjmp(ep_test_exception_jmp) == 0) {
        ...
        ret = ep_load_cache(env, enif_make_list(env, 4,
            ep_test_mk_msg_def(env, "ChildLink", enif_make_list(env, 1,
                ep_test_mk_field(env, st, "name", 2, 2, st->atom_bytes, st->atom_required))),
            ep_test_mk_msg_def(env, "FileChildren", enif_make_list(env, 1,
                ep_test_mk_field(env, st, "child_links", 1, 2,
                    enif_make_tuple2(env, st->atom_msg, enif_make_atom(env, "ChildLink")),
                    st->atom_repeated))),
            ep_test_mk_msg_def(env, "FuseResponse", enif_make_list(env, 1,
                enif_make_tuple4(env, make_atom(env, "gpb_oneof"), enif_make_atom(env, "fuse_response"),
                    enif_make_uint(env, 2),
                    enif_make_list2(env,
                        ep_test_mk_field(env, st, "file_children", 3, 2,
                            enif_make_tuple2(env, st->atom_msg, enif_make_atom(env, "FileChildren")),
                            st->atom_optional),
                        ep_test_mk_field(env, st, "xattr", 13, 2, st->atom_bytes, st->atom_optional))))),
            ep_test_mk_msg_def(env, "ServerMessage", enif_make_list(env, 1,
                enif_make_tuple4(env, make_atom(env, "gpb_oneof"), enif_make_atom(env, "message_body"),
                    enif_make_uint(env, 2),
                    enif_make_list(env, 1,
                        ep_test_mk_field(env, st, "fuse_response", 15, 2,
                            enif_make_tuple2(env, st->atom_msg, enif_make_atom(env, "FuseResponse")),
                            st->atom_optional)))))));
