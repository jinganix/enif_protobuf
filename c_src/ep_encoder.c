...
static inline enc_t *
enc_ensure(ErlNifEnv *env, enc_t *enc, size_t size)
{
...
stack_ensure(ErlNifEnv *env, stack_t *stack, spot_t **spot)
{
    if ((*spot) >= stack->end) {
        ...
        stack->tmp = _realloc(stack->mem, sizeof(spot_t) * (sizeof(spot_t) * stack->size * 2));
        if (stack->tmp == NULL) {
            return_exception(env, make_atom(env, "realloc_failed"));
        }

        *spot = stack->tmp + (*spot - stack->mem);
...
static inline void
do_pack_uint32(ErlNifEnv *env, uint32_t val, enc_t *enc)
{
    if (val >= 0x80) {
        *(enc->p)++ = val | 0x80;
        val >>= 7;
...
pack_uint32(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    uint32_t    val;
...
static inline ERL_NIF_TERM
pack_sint32(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    int32_t     val;
...
static inline ERL_NIF_TERM
pack_int32(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    int32_t     val;
...
static inline ERL_NIF_TERM
pack_fixed32(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    int32_t     val;
...
static inline void
do_pack_uint64(ErlNifEnv *env, uint64_t val, enc_t *enc)
{
    uint32_t hi = (uint32_t) (val >> 32);
    uint32_t lo = (uint32_t) val;

    if (hi == 0) {
        return do_pack_uint32(env, (uint32_t) lo, enc);
    }
...
static inline ERL_NIF_TERM
pack_uint64(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    uint64_t    val;
...
static inline ERL_NIF_TERM
pack_sint64(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    int64_t     val;
...
static inline ERL_NIF_TERM
pack_boolean(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    int32_t     val;
...
static inline ERL_NIF_TERM
pack_float(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    float       val;
    double      d_val;
    int32_t     i_val;
...
static inline ERL_NIF_TERM
pack_double(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    double      val;
    int64_t     l_val;
    state_t    *state = (state_t *) enif_priv_data(env);
...
static inline ERL_NIF_TERM
pack_utf8(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    uint32_t    val;
...
static inline ERL_NIF_TERM
pack_string(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    size_t          payload_len;
    uint32_t        len;
    ErlNifBinary    bin;
    ...
    if (enif_get_list_length(env, term, &len)) {
        ...
        do_pack_uint32(env, payload_len, enc);
        ...
        do_pack_uint32(env, bin.size, enc);
...
static inline ERL_NIF_TERM
pack_bytes(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc)
{
    ErlNifBinary    bin;
...
static inline ERL_NIF_TERM
pack_enum(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc, node_t *node)
{
    int32_t         val;
    enum_field_t   *field;

    if (enif_is_atom(env, term)) {
        field = bsearch(&term, node->fields, node->size, sizeof(enum_field_t), get_enum_compare_name);
        if (field == NULL) {
            return_error(env, term);
        }
        val = field->value;
...
static inline ERL_NIF_TERM
pack_tag(ErlNifEnv *env, uint32_t id, enc_t *enc)
{
    enc_ensure_default(env, enc);
...
field_t *
get_oneof_field(ErlNifEnv *env, ERL_NIF_TERM term, node_t *node, ERL_NIF_TERM *out)
{
    int32_t         arity;
    ERL_NIF_TERM   *array;
    ...
    *out = array[1];
    ...
    return bsearch(&(array[0]), node->fields, node->size, sizeof(field_t), get_field_compare_name);
}

static ERL_NIF_TERM
pack_element_packed(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc, field_t *field)
{
    ERL_NIF_TERM    ret;

    switch (field->f_type.type) {
    case field_sint32:
        check_ret(ret, pack_sint32(env, term, enc));
        break;

    case field_enum:
        check_ret(ret, pack_enum(env, term, enc, field->f_type.node));
        break;
...
static ERL_NIF_TERM
pack_field(ErlNifEnv *env, ERL_NIF_TERM term, enc_t *enc, field_t *field)
{
    char           *sentinel;
    ERL_NIF_TERM    ret;
...
encode(ErlNifEnv *env, ERL_NIF_TERM term, tdata_t *tdata)
{
    ...
    while (spot >= stack->mem) {
        ...
        if (spot->type == spot_tuple) {
            ...
            for (i = spot->pos; i < spot->node->size; i++) {

                term = spot->array[i];
                field = ((field_t *) (spot->node->fields)) + i;

                enc->omit = FALSE;
                if (field->o_type == occurrence_optional || field->f_type.type == field_oneof) {
                ...
                if (field->o_type == occurrence_repeated && field->opts.packed == TRUE) {
                    ...
                    *(enc->sentinel) |= WIRE_TYPE_LENGTH_PREFIXED;
                    ...
                    if (field->f_type.type == field_msg || field->f_type.type == field_map) {

                        spot++;
                        stack_ensure(env, stack, &spot);
                        spot->node = field->f_type.node;
                        if (spot->node == NULL) {
                            return_error(env, term);
                        }
                    ...
                    if (field->f_type.type == field_oneof) {
                        field = get_oneof_field(env, spot->array[i], field->f_type.node, &term);
                        if (field == NULL) {
                            return_error(env, spot->array[i]);
                        }
                    }
                    ...
                    if (field->f_type.type == field_msg || field->f_type.type == field_map) {
                        ...
                        *(enc->sentinel) |= WIRE_TYPE_LENGTH_PREFIXED;
                        ...
                        if (field->f_type.type == field_msg) {
                            (spot->array)++;
                            arity--;
                        }
