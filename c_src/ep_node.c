...
#define ensure_proper_list(env, list)                                     \
    do {                                                                  \
        ERL_NIF_TERM _ep_head, _ep_tail = (list);                         \
        if (!enif_is_list(env, _ep_tail)) {                               \
            raise_exception(env, list);                                   \
        }                                                                 \
        while (enif_get_list_cell(env, _ep_tail, &_ep_head, &_ep_tail)) { \
        }                                                                 \
        if (!enif_is_empty_list(env, _ep_tail)) {                         \
            raise_exception(env, list);                                   \
        }                                                                 \
    } while (0)
...
make_node(int fields_n, node_type_e n_type)
{
    ...
    if (fields_n > 0) {
        if (n_type == node_msg || n_type == node_oneof || n_type == node_group) {
...
parse_enum_fields(ErlNifEnv *env, ERL_NIF_TERM term, ep_node_t *node)
{
    ...
    ep_enum_field_t *field, *v_field, *vf, *name_field;
    ...
    while (enif_get_list_cell(env, term, &head, &tail)) {
        ...
        } else if (arity == 3 && array[0] == state->atom_option && array[1] == make_atom(env, A_ALLOW_ALIAS) && (array[2] == state->atom_true || array[2] == state->atom_false)) {
            term = tail;
            continue;
        } else {
            raise_exception(env, head);
        ...
        for (name_field = node->fields; name_field < field; name_field++) {
            if (name_field->name == field->name) {
                raise_exception(env, head);
            }
        }
...
parse_field_basic(ErlNifEnv *env, ERL_NIF_TERM term, ep_field_t *field)
{
    ...
    if (term == state->atom_int32) {
        field->type = field_int32;
        return RET_OK;
    } else if (term == state->atom_int64) {
        field->type = field_int64;
        return RET_OK;
    } else if (term == state->atom_uint32) {
        field->type = field_uint32;
        return RET_OK;
    } else if (term == state->atom_uint64) {
        field->type = field_uint64;
        return RET_OK;
    } else if (term == state->atom_sint32) {
        field->type = field_sint32;
        return RET_OK;
    } else if (term == state->atom_sint64) {
        field->type = field_sint64;
        return RET_OK;
    } else if (term == state->atom_fixed32) {
        field->type = field_fixed32;
        return RET_OK;
    } else if (term == state->atom_fixed64) {
        field->type = field_fixed64;
        return RET_OK;
    } else if (term == state->atom_sfixed32) {
        field->type = field_sfixed32;
        return RET_OK;
    } else if (term == state->atom_sfixed64) {
        field->type = field_sfixed64;
        return RET_OK;
    } else if (term == state->atom_bool) {
        field->type = field_bool;
        return RET_OK;
    } else if (term == state->atom_float) {
        field->type = field_float;
        return RET_OK;
    } else if (term == state->atom_double) {
        field->type = field_double;
        return RET_OK;
    } else if (term == state->atom_string) {
        field->type = field_string;
        return RET_OK;
    } else if (term == state->atom_bytes) {
        field->type = field_bytes;
        return RET_OK;
    }
        ...
        if (arity == 2 && array[0] == state->atom_enum && enif_is_atom(env, array[1])) {
            ...
            field->sub_node = NULL;
            return RET_OK;
        } else if (arity == 2 && array[0] == state->atom_group && enif_is_atom(env, array[1])) {
    ...
    if (term == state->atom_unknown) {
        field->type = field_unknown;
        return RET_OK;
    }
...
{
    ep_state_t *state = (ep_state_t *)enif_priv_data(env);
    int32_t arity, value_arity;
    ep_node_t *node;
    ERL_NIF_TERM *array, *value_array;
    ...
    if (array[1] != state->atom_int32 && array[1] != state->atom_int64 && array[1] != state->atom_uint32 && array[1] != state->atom_uint64 && array[1] != state->atom_sint32 && array[1] != state->atom_sint64 && array[1] != state->atom_fixed32 && array[1] != state->atom_fixed64 && array[1] != state->atom_sfixed32 && array[1] != state->atom_sfixed64 && array[1] != state->atom_bool && array[1] != state->atom_string) {
        raise_exception(env, array[1]);
    }

    if (enif_get_tuple(env, array[2], &value_arity, to_const(value_array)) && value_arity == 3 && value_array[0] == state->atom_map) {
        raise_exception(env, array[2]);
    }

    parse_msg_fields(env, enif_make_list2(env, enif_make_tuple7(env, state->atom_field, make_atom(env, "k"), enif_make_uint(env, 1), enif_make_uint(env, 2), array[1], state->atom_optional, enif_make_list(env, 0)), enif_make_tuple7(env, state->atom_field, make_atom(env, "v"), enif_make_uint(env, 2), enif_make_uint(env, 3), array[2], state->atom_optional, enif_make_list(env, 0))), node);
    return RET_OK;
}
...
parse_occurrence_type(ErlNifEnv *env, ERL_NIF_TERM term, ep_field_t *field)
{
    ...
    if (term == state->atom_required) {
        field->o_type = occurrence_required;
        return RET_OK;
    } else if (term == state->atom_optional) {
        field->o_type = occurrence_optional;
        return RET_OK;
    } else if (term == state->atom_defaulty) {
        field->o_type = occurrence_defaulty;
        return RET_OK;
    } else if (term == state->atom_repeated) {
        field->o_type = occurrence_repeated;
        return RET_OK;
    }
...
parse_opts(ErlNifEnv *env, ERL_NIF_TERM term, ep_field_t *field)
{
    ...
    while (enif_get_list_cell(env, term, &head, &tail)) {
        if (head == make_atom(env, A_PACKED)) {
            field->packed = TRUE;
        } else if (enif_get_tuple(env, head, &arity, to_const(array)) && arity == 2 && array[0] == make_atom(env, A_DEFAULT)) {
...
fill_msg_field(ErlNifEnv *env, ERL_NIF_TERM term, ep_field_t *field)
{
    ...
    if (!enif_get_uint(env, array[3], &(field->rnum))) {
        raise_exception(env, term);
    }
    (field->rnum)--;
    ...
    if (field->type == field_unknown) {
        field->fnum = 0;
    } else if (!enif_get_uint(env, array[2], &(field->fnum))) {
        raise_exception(env, term);
    }
...
parse_msg_fields(ErlNifEnv *env, ERL_NIF_TERM term, ep_node_t *node)
{
    ...
    while (enif_get_list_cell(env, term, &head, &tail)) {
        ...
        } else if ((arity == 4 || arity == 5) && array[0] == make_atom(env, A_ONEOF)) {
            if (!enif_is_atom(env, array[1])) {
                raise_exception(env, head);
            }
            ensure_proper_list(env, array[3]);
            if (arity == 5) {
                ensure_proper_list(env, array[4]);
            }
    ...
    if (node->v_size > 0) {
        ...
        for (i = 0; i < node->size; i++) {
            ...
            } else if (field->type != field_unknown) {
                ff->fnum = field->fnum;
                ff->field = field;
                ff++;
            }
...
parse_node(ErlNifEnv *env, ERL_NIF_TERM term, ep_node_t **node, uint32_t proto_v, ERL_NIF_TERM proto3_list)
{
    ...
    *node = NULL;
    ...
    if (msg_array[0] == state->atom_msg) {
        ...
        n_type = node_enum;
    } else if (msg_array[0] == state->atom_group) {
        n_type = node_group;
    } else {
        return RET_OK;
    }
    ...
    *node = make_node(len, n_type);
    ...
    if (proto3_list) {
        ...
        if (!enif_is_empty_list(env, proto3_list)) {
            raise_exception(env, proto3_list);
        }
    }
    ...
    if (n_type == node_msg || n_type == node_group) {
...
link_sub_node(ErlNifEnv *env, ep_cache_t *cache, ep_field_t *field)
{
    ...
    if (field->type == field_msg || field->type == field_map || field->type == field_enum || field->type == field_group) {
        if (field->sub_node == NULL) {
        ...
        while (spot >= stack->spots) {
            if (spot->type == spot_tuple) {
                ...
                for (j = spot->pos; j < (size_t)(spot->node->size); j++) {
                    ...
                    } else if (field->type == field_oneof || field->type == field_msg || field->type == field_map || field->type == field_group) {
                        if (spot_path_contains_node(spot, stack->spots, field->sub_node)) {
                            continue;
                        }
                        spot++;
